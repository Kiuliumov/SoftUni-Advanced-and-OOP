...
class RobotsManagingApp:
    ...
    def add_robot_to_service(self, robot_name: str, service_name: str):
        ...
        if type(robot_obj).__name__ != service_obj.POSSIBLE_SERVICE:
    ...
    def remove_robot_from_service(self, robot_name: str, service_name: str):
        service_obj = self._find_obj_by_name(service_name, self.services)
        robot_obj = next((r for r in service_obj.robots if r.name == robot_name), None)
    ...
    def _check_service_type(self, service_type):
        if service_type not in self.SERVICE_TYPES:
            raise ValueError(self.ERROR_MSG_TYPE_SERVICE)

    def _check_robot_type(self, robot_type):
        if robot_type not in self.ROBOT_TYPES:
            raise ValueError(self.ERROR_MSG_TYPE_ROBOT)
    ...
    def _find_obj_by_name(name, collection):
        obj = next((obj for obj in collection if obj.name == name), None)
        if obj is None:
